    ...
    if (!$vote) $doc->err(__('Поле "Вопрос" пусто'));
        ...
        foreach ($_POST as $key => $value) {
            ...
            if ($vv && preg_match('#^v([0-9]+)$#', $key, $m)) {
                if ($m[1] > 0 || $m[1] <= 10) $set[] = "`v$m[1]` = " . $db->quote($vv);
            }
        }
        $num = count($set);
        for ($i = 0; $i < (10 - $num); $i++)
        ...
        if (count($set) < 2) $doc->err(__('Должно быть не менее 2-х вариантов ответа'));
            ...
            if (!empty($_POST['finish'])) {
                ...
                $dcms->log('Форум',
                    'Закрытие голосования в теме [url=/forum/theme.php?id=' . $theme['id'] . ']' . $theme['name'] . '[/url]');
                ...
                $res->execute(Array($vote_a['id']));
                $dcms->log('Форум',
                    'Обнуление голосования в теме [url=/forum/theme.php?id=' . $theme['id'] . ']' . $theme['name'] . '[/url]');
                ...
                $dcms->log('Форум',
                    'Удаление голосования в теме [url=/forum/theme.php?id=' . $theme['id'] . ']' . $theme['name'] . '[/url]');

...
foreach ($cats AS $cat) {
    ...
    while ($i--) {
        ...
        $field = 'count_'.$i;
        $fields[$field] = "(SELECT COUNT(*) FROM `forum_views` AS `fv`
...
foreach ($cats AS $cat) {
    ...
    while ($i--) {
        ...
        $field = 'count_'.$i;
        $fields[$field] = "(SELECT COUNT(*) FROM `forum_messages` AS `fm`
 WHERE `fm`.`id_category` = $cat[id] AND `fm`.`time` >= $time_start AND `fm`.`time` < $time_end) AS $field";
    }

...
class CaptchaController extends Controller{
    ...
    public function captcha()
    {
        ...
        for ($i = 0; $i < 5; $i++) {
            $n = $code {$i};
            if ($png) {
                $num[$n] = imagecreatefrompng(H . '/public/images/captcha/' . $n . '.png');
            } elseif ($gif) {
                $num[$n] = imagecreatefromgif(H . '/public/images/captcha/' . $n . '.gif');
            } elseif ($jpg) {
                $num[$n] = imagecreatefromjpeg(H . '/public/images/captcha/' . $n . '.jpg');
            }
            imagecopy($img, $num[$n], $i * 10 + mt_rand(18,21), mt_rand(7,9), 0, 0, 15, 20);
        }
        
        // искажаем картинку
        $img = $this->multiWave($img);
     ...
     * @return gd2
     */
    private function multiWave($img) {
        $width = imagesx($img);
        $height = imagesy($img);

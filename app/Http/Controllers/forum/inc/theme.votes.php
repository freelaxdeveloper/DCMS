...
if ($theme['id_vote']) {
    ...
    if ($vote = $q->fetch()) {
        ...
        if (!empty($_GET['vote']) && App::user()->group >= $vote['group_vote'] && $vote_accept) {
            $vote_add = (int)$_GET['vote'];
            if ($vote['v' . $vote_add]) {
                $res = $db->prepare("INSERT INTO `forum_vote_votes` (`id_vote`, `id_user`, `vote`) VALUES (?,?,?)");
                $res->execute(Array($vote['id'], App::user()->id, $vote_add));
